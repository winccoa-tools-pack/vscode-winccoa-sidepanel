...
/**
 * Unified output channel for sidepanel extension with structured logging
 */
export class ExtensionOutputChannel {
    ...
    public static initialize(): vscode.OutputChannel {
        if (!ExtensionOutputChannel.instance) {
            ExtensionOutputChannel.instance = vscode.window.createOutputChannel('WinCC OA Sidepanel');
        }
    ...
    public static updateLogLevel(): void {
        const config = vscode.workspace.getConfiguration('winccoaSidepanel');
